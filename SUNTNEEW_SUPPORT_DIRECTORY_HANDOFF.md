...
3. **产品插页并入对应手册**：`00_收件箱/2026-09/02说明书插页` 的制造商信息＋多语言警示插页
   已合并在 RV 电池四款、KB700、U23、U32、OJ02、A20、A3 手册的最后两页；站内不单独展示插页。
   KB700 / U32 / OJ02 / C2 四份插页原件由 `tools/manual-build/build-inserts.py` 生成：
   版式沿用搭电宝家族原稿，只替换型号、制造商与地址、E-mail/Tel（统一 `info@suntneew.com`）
   和 Distributor（按营业执照写作 `Shanghai Shunxiangyang Clean Energy Technology Co., Ltd.`）。
   C2 只输出 PDF（`00_收件箱/2026-09/02说明书插页/搭电宝说明书插页/C2说明书插页_更新版_20260911.pdf`），
   站内没有对应产品，因此不进入任何页面或手册。
...
| 四款房车电池 | 41 | 1 | 22 | 8 | 8 | 2 |

每份手册的所有页面都统一成 A4：英文原页（小开本）与插页等比缩放居中，法文、德文正文按 A4 重新分页，
这样下载后的页面尺寸一致，打印时也不会出现文字贴到纸边的情况。
