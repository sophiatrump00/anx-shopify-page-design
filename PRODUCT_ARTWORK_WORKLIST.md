...
本清单服务于五个已建产品详情页：A20、A3、SuntNeew VH10A/VH15A、SuntNeew WL5A、SuntNeew WL10B。
...
| 命名 | `brand-model-scene-vN.ext`，例如 `suntneew-wl5a-install-v2.png` |
| 图片内文案 | 默认不放技术参数、价格、认证图标或长文案；页面 HTML 负责这些可变信息 |
| 商标与外观 | 产品型号、外观、接口和颜色以参考源图为准；新发布的家储主图库统一使用 SuntNeew 身份；已保留的旧 ENERGY STAR 图片作为附加/历史参考媒体，不删除 |
...
| P0 | WL5A | Features「System fit」 | `assets/suntneew-wl5a-install-v2.png` | 紧凑型家庭储能安装环境 | SuntNeew 标识、低压电池体量和安装环境可信；不作为接线教程 |
| P0 | WL10B | Features「More stored energy」 | `assets/suntneew-wl10b-install-v2.png` | 更大容量单机在家庭能源间的场景 | SuntNeew 身份、较高机身比例、配电柜与导管合理；无参数文字 |
| P0 | VH10A/VH15A | Features「Modular high voltage」 | `assets/suntneew-vh-install-v2.png` | 高压模块系统的专用能源间场景 | 深灰模块层级、显示屏和绿色指示条可信；无虚构系统认证或数值 |
...
### SuntNeew VH10A / VH15A

| 优先级 | 图号/页面模块 | 画面任务 | 现有来源或候选 | 美工交付与验收 |
| --- | --- | --- | --- | --- |
| P0 | Gallery + Features：项目安装 | 深灰高压模块系统在独立能源间的安装环境 | `suntneew-vh-install-v2.png` | SuntNeew 身份正确；不出现假品牌、假认证、虚构 kWh/电压 |
| P0 | Features：两组/三组模块 | VH10A 与 VH15A 的模块层级对比 | `suntneew-vh10-gallery-01-v2.jpg` | 模块数与选中变体一致；用于解释配置而不是替代规格字段 |
| P1 | Features：控制与结构 | 显示屏、指示条、柜体细节与背部接口 | `suntneew-vh10-gallery-04-v2.jpg` 至 `06.jpg` | 只用已核对结构；不展示无来源的内部构造 |
...
| 优先级 | 图号/页面模块 | 画面任务 | 现有来源或候选 | 美工交付与验收 |
| --- | --- | --- | --- | --- |
| P0 | Gallery + Features：家庭安装 | 紧凑低压电池与能源柜的真实家庭能源间 | `suntneew-wl5a-install-v2.png` | SuntNeew 标识正确，安装比例可信；不写电气性能结论 |
| P0 | Features：产品主视觉 | 白色正面、侧面和背部的干净产品展示 | `suntneew-wl5a-main-v2.jpg`、`suntneew-wl5a-gallery-01-v2.jpg`、`suntneew-wl5a-gallery-04-v2.jpg` | 主体完整，接口和安装结构不被裁坏 |
...
| 优先级 | 图号/页面模块 | 画面任务 | 现有来源或候选 | 美工交付与验收 |
| --- | --- | --- | --- | --- |
| P0 | Gallery + Features：更高容量安装 | 较高机身在家庭能源间、与家庭空间连接的场景 | `suntneew-wl10b-install-v2.png` | SuntNeew 标识、机身高宽比例和环境可信；无假参数 |
| P0 | Features：产品主视觉 | 白色前视、侧视和空间占用感 | `suntneew-wl10b-main-v2.jpg`、`suntneew-wl10b-gallery-01-v2.jpg` | 不将 WL5A 机身误用于 WL10B |
...
> 2026-09-01：WL5A、WL10B、VH10A/VH15A 的 18 张旧 ENERGY STAR 产品图片已恢复并保留在 Shopify 产品图库末尾；每个产品的 7 张 SuntNeew 新图仍作为主序列使用。
...
- [ ] 家储新主图库和主题素材使用 SuntNeew 身份；保留的旧 ENERGY STAR 附加媒体不作为新品牌主视觉或新增认证声明。
